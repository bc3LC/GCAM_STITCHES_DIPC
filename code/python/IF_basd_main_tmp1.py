...
class Args:
    task_id = 12         # 0 & 12: CanESM5  --  BASE & CLIM;           1: MRI-ESM2-0
    run_name = 'heat_29012026'
    # run_name = 'heat_ineq'
...
with LocalCluster(processes=True, threads_per_worker=1, n_workers = 20, memory_limit = "6GB") as cluster, Client(cluster) as client:
    # Setting up dask.Client so that I can ssh into the dashboard
    port = client.scheduler_info()['services']['dashboard']
    host = client.run_on_scheduler(socket.gethostname)
    print("If running remotely use the below command to ssh into dashboard from a local terminal session")
