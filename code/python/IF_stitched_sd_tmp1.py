...
###### extra step to save memory #########
obs_reference_data = obs_reference_data.chunk({'time': 500, 'lat': 100, 'lon': 100})
sim_application_data = sim_application_data.chunk({'time': 500, 'lat': 100, 'lon': 100})
# obs_reference_data = obs_reference_data.astype('float32')
# sim_application_data = sim_application_data.astype('float32')
# obs_reference_data = obs_reference_data.persist()
# sim_application_data = sim_application_data.persist()
###### end extra step #########
...
if not os.path.exists(obs_clean_path):
    obs_reference_data.astype('float32').chunk({'time': 500, 'lat': 50, 'lon': 50}).to_netcdf(obs_clean_path)


if not os.path.exists(sim_clean_path):
    sim_application_data.astype('float32').chunk({'time': 500, 'lat': 50, 'lon': 50}).to_netcdf(sim_clean_path)


with LocalCluster(n_workers=1, threads_per_worker=30, memory_limit="150GB") as cluster, Client(cluster) as client:    
    # 3. CRITICAL: Re-chunk to much smaller spatial blocks 
    # but KEEP them in the workers (persist)
    # print("Persisting data to worker memory...")
    # obs_ref = obs_reference_data.chunk({'time': 500, 'lat': 50, 'lon': 50}).persist()
    # sim_app = sim_application_data.chunk({'time': 500, 'lat': 50, 'lon': 50}).persist()    
    # # Wait for the persistence to finish so the RAM is already 'loaded'
    # from dask.distributed import wait
    # wait([obs_ref, sim_app])
    # print("Data ready in RAM.")
    # # 4. Now run the problematic function
    # print("Starting SD Initialization...")
    # ds = basd.init_downscaling(obs_ref, sim_app, 
...
# # Save it
# with open('ds_data.pkl', 'wb') as f:
#     pickle.dump(ds, f)

# # Load it later
# with open('ds_data.pkl', 'rb') as f:
#     ds_reloaded = pickle.load(f)



# 10. Run downscaling
# Initialize downscaling
with LocalCluster(n_workers=2, threads_per_worker=8, memory_limit="80GB") as cluster, Client(cluster) as client:
# with LocalCluster(n_workers=5, threads_per_worker=6, memory_limit="30GB") as cluster, Client(cluster) as client:
    dask.config.set({
        "distributed.worker.memory.target": 0.7,  # Spill at 70%
        "distributed.worker.memory.spill": 0.85, # Aggressive at 85%
        "distributed.worker.memory.pause": 0.9,  # Stop at 90%
    })
    print(client.dashboard_link) 
    print(socket.gethostname())
    # obs_reference_data = obs_reference_data.persist()
    # sim_application_data = sim_application_data.persist()
    print("SD Initialization Beginning")
...
def load_ba_data(run_object):
    ...
    # If necessary, fix freq
    diffs = np.diff(sim_data.time.values)
    if len(np.unique(diffs)) > 1:
        print("⚠️ Irregular time steps detected — rebuilding daily time axis...")
        # Create a clean daily range
        new_time = pd.date_range(
            start=str(sim_data.time.values[0]),
            end=str(sim_data.time.values[-1]),
            freq='D'
        )
        # Reindex to the new daily time axis
        sim_data = sim_data.reindex(time=new_time)
        # (Optional) fill missing values by interpolation
        # sim_data = sim_data.interpolate_na(dim='time')
