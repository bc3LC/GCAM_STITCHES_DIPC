...
from IF_setup import IF_PATH, experiment_name, grid_cell_area, remove_nas
...
# INPUTS
pred_df = pd.read_csv(os.path.join(IF_PATH, "output/historic_glm/Predictors_CSV_Mean_2019.csv"))    
hist_nc = xr.open_dataset(os.path.join(IF_PATH, "output/historic_glm/Predictions_NC_Annual_2019.nc"))
...
def master_csv(hist_nc, scenarios: list, esms: list):
    ...
    if "MME" in esms: 
        time_series.to_csv(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_ESA_MME.csv"))
        master_nc.to_netcdf(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_ESA_MME.nc"))
    else:
        time_series.to_csv(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_ESA.csv"))
        master_nc.to_netcdf(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_ESA.nc"))
...
def regional_csv(hist_nc, regions, scenarios: list, esms: list):
    ...
    if "MME" in esms: 
        time_series.to_csv(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_Reg_ESA_MME.csv"))
    else:
        time_series.to_csv(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_Reg_ESA.csv"))
