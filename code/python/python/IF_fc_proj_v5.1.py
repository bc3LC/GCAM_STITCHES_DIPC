...
gfed_fc = xr.open_dataset(os.path.join(IF_PATH, "input/historic_glm/GLM_Historic_Inputs_v5.1_v2.nc"))[["FC_for_GFED", "FC_nonf_GFED", "FC_total_GFED"]]
...
def fuel_consumption(scenario, esm, fuel_load, biome_nc, gfed_fc, regions):
    ...
    ###########################################################################
    # Load the scenario-specific scaled CC 
    ...
    ###########################################################################
    # Boreal - Add following Park et al. 2023 default FC for peat fires in boreal forests of 2200 gC.m2 
    ...
    ###########################################################################
    # Apply the change ratio of dynamic FC to static GFED layer at 2019
    
    # 0. Lighten the final NETCDF
    fe_param_final = fe_param_boreal[["FC_Forest", "FC_NonForest"]]
    
    # 1. Transform the projection to change ratio through time (after 2019 only)
    ...
    # 2. Apply that to FC_GFED at the year 2019
    ...
    gfed_fc_proj.to_netcdf(os.path.join(fc_dir, f"FC_{scenario}_{esm}_2019-2100_proj_v5.1.nc"))
...
if __name__ == "__main__":
    ...
    # # Run for the MME of each scenario 
    # for scenario in scenarios: 
    #     print(f"Processing MME of {scenario}")
    #     vpd_pattern = f'MME_STITCHES_W5E5v2_{scenario}_vpd_global_monthly_{start_year}_{end_year}.nc'        
    #     vpd = xr.open_dataset(os.path.join(climate_dir, vpd_pattern))
    #     vpd = vpd.resample(time="YE").mean()
    #     vpd["vpd"].mean(dim="time").plot()
    #     vpd = vpd.reindex(lat=list(reversed(vpd.lat)))
    #     print(vpd)
    #     cc_scaling(cc_range_table, vpd, scenario, "MME") # Takes the VPD projection and computes the CC. Output: CC projection 
    #     fuel_consumption(scenario, "MME", fuel_load, biome_nc, gfed_fc, regions) # Takes the CC and the FL dataset and computes FC. Output: FC projection
    #     print(f"FC Completed for MME of {scenario}")
    # print("FC Completed for all scenarios")
    ...
    #         basd_path = os.path.join(basd_dir, f'{esm}', f'{scenario}')
    #         print(f'BASD directory for {esm} with scenario {scenario} being deleted from /scratch')
    #         try:
    #             shutil.rmtree(basd_path)
    #         except OSError as e:
    #             print(f"Error removing BASD directory: {e}")
