...
# INPUTS
pred_df = pd.read_csv(os.path.join(IF_PATH, "output/historic_glm/Predictors_CSV_Mean_2019_v5.1.csv"))    
hist_nc = xr.open_dataset(os.path.join(IF_PATH, "output/historic_glm/Predictions_NC_Annual_2019_v5.1.nc"))
...
def glm_train(pred_df, formula: str):
    ...
    # # # DEBUG
    # pred_df = pd.read_csv(os.path.join(INPUT_PATH, f"GLM_MODEL/Predictors_CSV_Mean_2019.csv"))
...
def glm_predict(model, regions, scenario: str, esm: str, years: list):
    ...
    forest_prop = xr.open_dataset(os.path.join(IF_PATH, f"output/historic_glm/BA_Forest_Proportion_{start}-{end}_v5.1.nc"))
    
    # Open the fuel consumption data from VPD scaling 
    fc_param = xr.open_dataset(os.path.join(fc_dir, f"FC_{scenario}_{esm}_2019-{end}_proj_v5.1.nc"))
    ...
    # # From monthly mean to annual total (w/ Chen BA)
    # pred_df["BA_frac_pred"] = pred_df["BA_frac_pred"] * 12        
    ...
    # Check the forest area on average across 2015-2100
    # print("The average forest area is", (pred_df["forest"] * pred_df["Grid_area"]).sum()/86) 

    # Save CSV with BA prediction    
    pred_df.to_csv(os.path.join(bace_dir, f"BA_Prediction_{scenario}_{esm}_v5.1.csv"))
    # print(pred_df)
    
    # Read it again to avoid the index issue 
    pred_df = pd.read_csv(os.path.join(bace_dir, f"BA_Prediction_{scenario}_{esm}_v5.1.csv"))
    ...
    pred_netcdf.to_netcdf(os.path.join(bace_dir, f"BA_CE_Prediction_{scenario}_{esm}_v5.1.nc"))
    print(f"GLM Prediction completed for {scenario} and {esm}")
    
    # ##########################################################################
    # # 6. ADD REGIONS HACK 
    # regions = xr.open_dataset(os.path.join(data_dir, "GFED_Regions.nc"))
    # pred_netcdf = xr.open_dataset(os.path.join(results_dir,f"burned_area/BA_FE_Prediction_{scenario}_{esm}.nc"))
    # pred_netcdf = xr.merge([pred_netcdf, regions])
    # print(f"{scenario} - {esm}")
    # print(pred_netcdf)
    # pred_netcdf.to_netcdf(os.path.join(results_dir,f"burned_area/BA_FE_Prediction_{scenario}_{esm}.nc"))
...
def master_csv(hist_nc, scenarios: list, esms: list):
    ...
    hist_mean["FC_total_GFED"] = (
        (hist_annual["FC_for_GFED"] * hist_annual["BA_area_for"] +
         hist_annual["FC_nonf_GFED"] * hist_annual["BA_area_nonf"])
        .groupby("time").sum() /
        (hist_annual["BA_area_for"] + hist_annual["BA_area_nonf"]).groupby("time").sum()
    )
    ...
    for scenario in scenarios: 
        for esm in esms: 
            ...
            pred_nc = xr.open_dataset(os.path.join(bace_dir, f"BA_CE_Prediction_{scenario}_{esm}_v5.1.nc"))
            ...
            pred_mean["FC_total_GFED_proj"] = (
                (pred_nc["FC_for_GFED_proj"] * pred_nc["BA_area_for_pred"] +
                 pred_nc["FC_nonf_GFED_proj"] * pred_nc["BA_area_nonf_pred"])
                .sum(dim=["lat", "lon"]) /
                (pred_nc["BA_area_for_pred"] + pred_nc["BA_area_nonf_pred"]).sum(dim=["lat", "lon"])
            )
    ...
    if "MME" in esms: 
        time_series.to_csv(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_ESA_MME_v5.1.csv"))
        master_nc.to_netcdf(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_ESA_MME_v5.1.nc"))
    else:
        time_series.to_csv(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_ESA_v5.1.csv"))
        master_nc.to_netcdf(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_ESA_v5.1.nc"))

def regional_csv(hist_nc, regions, scenarios: list, esms: list):

    ##########################################################################
    # 1.A Open the historical data (NC without predictions)
    # hist_nc = xr.open_dataset(os.path.join(data_dir, f"Predictors_ALL_Annual_{glm_lu_source}_Pred_GFED_2.nc"))
    # hist_nc = xr.open_dataset(os.path.join(data_dir, f"Predictors_ALL_Annual_{glm_lu_source}_Pred_ECMWF.nc"))
    # hist_nc = xr.open_dataset(os.path.join(data_dir, f"BAFE_Predictors_FINAL.nc"))
    ...
    # # # DEBUG
    # hist_nc = xr.open_dataset(os.path.join(INPUT_PATH, f"GLM_MODEL/BAFE_Predictors_FINAL.nc"))
    ...
    for scenario in scenarios: 
        ...
        for esm in esms: 
            ...
            # # DEBUG
            # pred_nc = xr.open_dataset(os.path.join(INPUT_PATH, "../results/DIPC/BA_FE_Prediction_SSP3-6p6_MPI-ESM1-2-LR.nc"))
            # pred_nc = xr.merge([pred_nc, regions])
    ...
    master_full = [hist_mean, master_df]
    time_series = pd.concat(master_full)
    # master_nc = xr.concat(master_nc, dim = "scen_esm_reg") 

    print(time_series)    
    
    if "MME" in esms: 
        time_series.to_csv(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_Reg_ESA_MME_v5.1.csv"))
    else:
        time_series.to_csv(os.path.join(bace_dir, f"BA_CE_Prediction_AllScen_Reg_ESA_v5.1.csv"))
...
def factorial_decomp(model, regions, scenario: str, esm: str, years: list, option: int, save = True):
    ...
    # # DEBUG
    # pred_nc = xr.open_dataset(os.path.join(INPUT_PATH, f"../results/DIPC/BA_FE_Prediction_SSP1-2p6o_MPI-ESM1-2-LR.nc"))
    ...
    if option == 1: 
        ...
        for group_name, variables in factor_groups.items():
            ...
            for variable in variables:
                # base_sum = pred_df[variable].groupby(level="time").mean()
                # plt.figure(figsize=(8, 3))
                # plt.plot(base_sum.index, base_sum.values, label="Base", alpha=0.7)
                
                pred_df[variable] = idx_latlon.map(map_2020[variable])
                
                # fixed_sum = pred_df[variable].groupby(level="time").mean()
                # plt.plot(fixed_sum.index, fixed_sum.values, label="Fixed", alpha=0.7)
                # plt.title(f"{variable} — {group_name}")
                # plt.legend()
                # plt.tight_layout()
                # plt.show()
    ...
    if option == 2: 
        ...
        for group_name, variables in factor_groups.items():
            ...
            for variable in free_vars:
                ...
                # fixed_sum = pred_df[variable].groupby(level="time").mean()
                # plt.plot(fixed_sum.index, fixed_sum.values, label="Fixed", alpha=0.7)
                # plt.title(f"{variable} — Fixed (not {group_name})")
                # plt.legend()
                # plt.tight_layout()
                # plt.show()
...
if __name__ == "__main__":
    ...
    for i, scenario in enumerate(scenarios):
        ...
        # # Run the predict and visualize for the MME 
    ...
    # esms = ["MME"] # ADD THE MEAN AND SD FOR THE MME ACTUALLY THERE DIRECTLY 
    # master_csv(hist_nc, scenarios, esms)
