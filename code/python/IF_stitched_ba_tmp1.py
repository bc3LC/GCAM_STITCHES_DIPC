...
# # DEBUG -- ADDED CLUSTER 
# Configure Dask to be less aggressive with heartbeats on busy clusters
...
cluster_kwargs = {
    ...
    "processes": True,
    "memory_limit": "8GB",
    "scheduler_kwargs": {"idle_timeout": "2h"},
...
with LocalCluster(**cluster_kwargs) as cluster, Client(cluster) as client:
    ...
    # Enable Active Memory Manager to reduce spilling to disk
    client.amm.start() 
    # 9. Bias Adjustment Execution
    ba = basd.init_bias_adjustment(
        obs_reference_data, 
        sim_reference_data, 
        sim_application_data,
        run_object.Variable, 
        params,
        lat_chunk, 
        lon_chunk,
