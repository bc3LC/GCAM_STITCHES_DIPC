...
if __name__ == "__main__":
    ...
    if stitched:
        with open(os.path.join(IF_PATH, intermediate_path, run_name, 'stitch_array.job'), 'w') as job_file:
            ...
            job_file.writelines('module purge\n')
            job_file.writelines('module load GCC/11.3.0\n')
            job_file.writelines('module load Miniforge3\n')
            job_file.writelines('# activate conda environment\n')
            job_file.writelines('conda deactivate\n')
            job_file.writelines('source /scicomp/EasyBuild/CentOS/7.4.1708/Skylake/software/Miniforge3/24.1.2-0/etc/profile.d/conda.sh\n')
            ...
            job_file.writelines(f"python /scratch/bc3lc/heat-deaths/GCAM_STITCHES_DIPC/code/python/IF_generate_stitched_data_array.py $SLURM_ARRAY_TASK_ID {run_name}\n\n")
    ...
    with open(os.path.join(IF_PATH, intermediate_path, run_name, 'basd.job'), 'w') as job_file:
        ...
        job_file.writelines('start=`date +%s.%N`\n\n')
        job_file.writelines('# Run script\n')
        job_file.writelines(f"python /scratch/bc3lc/heat-deaths/GCAM_STITCHES_DIPC/code/python/IF_basd_main.py $SLURM_ARRAY_TASK_ID {run_name}\n\n")
        job_file.writelines('# End timing and print runtime\n')
        job_file.writelines('end=`date +%s.$N`\n')
        job_file.writelines('runtime=$( echo "($end - $start) / 60" | bc -l )\n')
        job_file.writelines('echo "Run completed in $runtime minutes"\n')
    ...
    with open(os.path.join(intermediate_path, run_name, 'manager.job'), 'w') as job_file:
        ...
        job_file.writelines(f"stitch_id=$(sbatch --parsable /scratch/bc3lc/heat-deaths/GCAM_STITCHES_DIPC/intermediate/{run_name}/stitch_array.job)\n\n")
            
        job_file.writelines('# Run bias adjustment and downscaling\n')
        job_file.writelines(f"basd_id=$(sbatch --parsable --dependency=afterok:$stitch_id /scratch/bc3lc/heat-deaths/GCAM_STITCHES_DIPC/intermediate/{run_name}/basd.job '{run_name}')\n\n")
